...
namespace JimBobBennett.MvvmLight.AppCompat
{
    public class AppCompatNavigationService : INavigationService
    {
        ...
        readonly Dictionary<string, object> _parametersByKey = new Dictionary<string, object>();

        const string RootPageKey = "-- ROOT --";
        const string ParameterKeyName = "ParameterKey";
                ...
                lock (_pagesByKey)
                {
                    if (!_pagesByKey.ContainsKey(pageKey))
                        throw new ArgumentException($"No such page: {pageKey}. " +
                                                    "Did you forget to call NavigationService.Configure?", nameof(pageKey));
